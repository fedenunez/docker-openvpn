...
# gen TCP client
docker run -v $OVPN_DATA:/etc/openvpn --rm -e "EASYRSA_BATCH=1" $IMG easyrsa build-client-full $CLIENT_TCP nopass
...
docker run -v $OVPN_DATA:/etc/openvpn --rm -e "EASYRSA_BATCH=1" $IMG easyrsa build-client-full $CLIENT_UDP nopass
